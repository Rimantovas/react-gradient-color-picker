...
import type { ColorsProps, Config, GradientProps } from '../shared/types.js';
import { rgb2cmyk } from '../utils/converters.js';
import { formatInputValues, getColors, high, low } from '../utils/formatters.js';
...
export const useColorPicker = (
  ...
  const createGradientStr = (newColors: GradientProps[]) => {
    ...
    const colorString = sorted?.map((cc: ColorsProps) => `${cc?.value} ${cc.left}%`);
  ...
  const deletePoint = (index: number) => {
    if (colors?.length > 2) {
      ...
      const remaining = colors?.filter((rc: ColorsProps, i: number) => i !== pointToDelete);
